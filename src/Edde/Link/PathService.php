...
class PathService {
	...
	public function convert($path, &$keys = [], array $options = []) {
		$strict = $options['strict'] ?? false;
		$end = $options['end'] ?? true;
		$start = $options['start'] ?? true;
		$flags = !empty($options["sensitive"]) ? "" : "i";
		$index = 0;
		...
		if (is_array($path)) {
			...
			$path = array_map(function ($value) use (&$keys, &$options) {
				return $this->getRegexpSource($this->convert($value, $keys, $options));
			}, $path);
		...
		return '/' . ($start ? '^' : '(?<start>.*?)') . $path . ($end ? '$' : '') . '/' . $flags;
	}
}
